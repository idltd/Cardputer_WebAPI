...
export function initGpio(el) {
    ...
    panel.querySelector('#gpio-mode-btn').onclick = () => {
        ...
        currentApi.post(`/api/gpio/${pin}/mode`, { mode }).then(r => {
            addLog(`Pin ${r.pin} mode set to ${r.mode}`);
    ...
    panel.querySelector('#gpio-write-btn').onclick = () => {
        if (!currentApi) return;
        const pin = panel.querySelector('#gpio-pin').value;
        currentApi.post(`/api/gpio/${pin}`, { value: writeVal }).then(r => {
            addLog(`Pin ${r.pin} written ${r.value ? 'HIGH' : 'LOW'}`);
        }).catch(e => addLog(`Error: ${e.message}`));
    };
